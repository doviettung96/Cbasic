...
void delete(tree_type *tree, char *model)
{
	int flag = 0;
	if (*tree != NULL)
	{
		flag = strcmp(model, (*tree)->info.model);

		if (flag < 0)
			delete(&(*tree)->left, model);
		else if (flag > 0)
			delete(&(*tree)->right, model);
...
tree_type search(tree_type tree, char *model) {
	int flag = 0;
	if (tree == NULL)
	{
		printf("Not found!!\n");
	...
	flag = strcmp(tree->info.model, model);
	if (flag == 0) {
		printf("Found.\n");
		return tree;
	}
		...
		tree = search(tree->left, model);
	else if (flag < 0)
		tree = search(tree->right, model);
}
...
void insert(tree_type *tree, element_type entry) {
	...
	flag = strcmp((*tree)->info.model, entry.model);
	if (flag == 0)
	{
